...
typedef struct MenuEntProc MenuEntProc;
struct MenuEntProc
{
    /* 00 */ PROC_HEADER;
...
extern struct MenuInfo MenuInfo_UnitMenu; // FE6 0x203956C

struct MenuProc* StartMenu_AndDoSomethingCommands(
	struct MenuInfo const* info, 
	int x, 
	int left, 
	int right); // FE6 8042018+1
	
struct MenuProc* StartMenuAdjusted(
	const struct MenuInfo* def, 
	int xSubject, 
	int xTileLeft, 
	int xTileRight); // FE6 80417E8+1
	
struct MenuProc* StartMenu(
	struct MenuInfo const* info, 
	struct MenuRect rect, 
	ProcPtr parent); // FE6 8041844+1
